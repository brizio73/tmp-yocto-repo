...
#define DEVNAME "NEXCOM_SMBus"

static unsigned char *prbuf, *pwbuf;

static int NEXCOM_SMBus_open (struct inode *inode, struct file *filp) {
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 0)
...
static int NEXCOM_SMBus_close(struct inode *inode, struct file *file) {
...
static int NEXCOM_SMBus_read (struct file *filp, char *buf, size_t count, loff_t *t) {
	char value;
...
static int NEXCOM_SMBus_write (struct file *filp, const char *buf, size_t count, loff_t *t) {
	char value;
...
struct file_operations tg_fops = {
	...
	.write = NEXCOM_SMBus_write,
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 36)
	.unlocked_ioctl = NULL, /* ioctl */
#else
	.ioctl = NULL, /* ioctl */
#endif
	.open = NEXCOM_SMBus_open,
	.release = NEXCOM_SMBus_close
};
